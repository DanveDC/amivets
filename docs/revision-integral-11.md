...
| Ficha de mascota | Ya elevado ("idioma visual", 9b59b45) | Sin cambios | **Desvío estructural confirmado, no corregido.** `FichaMascota.html` es una ficha de un paciente (header + tabs + timeline + panel de constantes/alertas/actividad). La pantalla real (`sec-consultorio`, "Historia clínica") es un layout de 3 columnas con lista maestra de pacientes + tabs de íconos — arquitectura de información distinta, no solo otro estilo. Ver §6 y pendientes. |
| Insumos | Ya elevado ("idioma visual", 9b59b45) | Sin cambios | **Desvío estructural confirmado, no corregido.** El boceto trae KPI (materiales activos/bajo mínimo/por reponer/valor del inventario) + barras de nivel de stock + pills de categoría. La pantalla real es una tabla plana sin nada de eso, con columnas distintas (Vencimiento/Acciones en vez de Nivel/Costo). |
| Catálogo | Ya elevado ("idioma visual", 9b59b45) | Sin cambios | **Desvío estructural confirmado, no corregido.** El boceto es master-detail (lista de servicios + panel de detalle con insumos consumidos e historial de precios). La pantalla real es una tabla plana ID/Nombre/Categoría/Precio/Unidad/Activo, sin detalle. |
| Facturación | Ya elevado ("idioma visual", 9b59b45) | Sin cambios | **Desvío estructural confirmado, no corregido.** El boceto es un checkout por orden (conceptos a cobrar, forma de pago, abonos, "Emitir factura"). La entrada de menú real es un historial/listado de facturas ya emitidas; "Facturar orden" (desde Orden abierta) crea la factura directo por API con `total_pagado:0.0`, sin ningún paso de selección de forma de pago — el flujo del boceto no existe en ningún lado de la app tal cual está dibujado. |
...
usan la tipografía/paleta/tokens correctos — pero ninguna de las cuatro sigue
la *composición* de su boceto. Llevar las cuatro a fidelidad estructural real
es un trabajo del mismo orden que el que llevó Reportes en esta tarea,
multiplicado por cuatro. Avisé de esto durante la tarea (mensaje a quien la
encargó) y decidí, a falta de otra indicación, documentar el gap en detalle
acá y no tocar la estructura de esas cuatro pantallas — corregir eso es un
cambio de alcance, no un hallazgo de revisión puntual.
...
**Specs nuevos/ampliados agregados en esta tarea** (todos en `shell.spec.js`
y `auth.spec.js`, no se crearon archivos nuevos — el recorrido que pide el
enunciado ya está cubierto, ver abajo):
...
adjunto), y el propio `shell.spec.js` (panel del día → orden → anexar →
confirmar → bandeja del gestor → tomar → ejecutar, a nivel UI). No armé un
...
**Resultado final** (corrida completa en 2 tandas, después de todos los
cambios de esta tarea):

- Tanda 1 (9 specs): 65 passed *(incluye el test de foco/teclado nuevo de
  `auth.spec.js`)*.
- Tanda 2 (10 specs): 86 passed + 2 skipped *(incluye los 4 tests/
  assertions nuevos de `shell.spec.js`)*.
- **Total: 151 passed + 2 skipped = 153 tests, 0 failed.**
...
|---|---|
| Insumos, Catálogo, Facturación, Ficha de mascota — fidelidad estructural al boceto | Cuatro rediseños del mismo tamaño que el de Reportes en esta tarea. Insumos necesita KPI de stock + barras de nivel; Catálogo necesita master-detail con historial de precios visible; Facturación necesita el flujo de checkout por orden con forma de pago (hoy "Facturar orden" crea la factura directo, sin ese paso); Ficha de mascota necesita la vista de un paciente con tabs, en vez del layout de 3 columnas actual. Ninguno toca el modelo de datos — es todo de front + reorganizar endpoints existentes. |
